...
if (strlen($act) == 0 || $act == "del" || $act == "edit") {
  ...
  while ($o = $result->fetch_object()) {
    ...
    echo "<tr><form action='' method=post>
      ...
      <td><input name=CustomerID size=40 value='". $o->CustomerID . "' style='background-color:#dddddd' readonly></td>
      <td><input name=EmployeeID size=20 value='". $o->EmployeeID . "' style='background-color:#dddddd' readonly></td>
      <td>";
...
if ($act == "add") {
   $connect = new mysqli($host,$user,$password,$db);
   $result = $connect->query("insert into suppliers (OrderID,CustomerID,EmployeeID) values('". $_REQUEST{'OrderID'} . "','". $_REQUEST{'CustomerID'} . "','". $_REQUEST{'EmployeeID'} . "')");   
...
if ($act == "del : confirm") {
  $connect = new mysqli($host,$user,$password,$db);
   $result = $connect->query("delete from suppliers where OrderID ='". $_REQUEST{'OrderID'} . "'");   
...
if ($act == "edit : confirm") {
  $connect = new mysqli($host,$user,$password,$db);
  $result = $connect->query("update $tb set CustomerID ='". $_REQUEST{'CustomerID'} . "',  EmployeeID ='". $_REQUEST{'EmployeeID'} . "' where OrderID =" . $_REQUEST{'OrderID'});      
