...
if ($act == "add") {
   $connect = new mysqli($host,$user,$password,$db);
   $result = $connect->query("insert into suppliers (SupplierID,CompanyName,ContactName) values('". $_REQUEST{'SupplierID'} . "','". $_REQUEST{'CompanyName'} . "','". $_REQUEST{'ContactName'} . "')");   
...
if ($act == "del : confirm") {
  $connect = new mysqli($host,$user,$password,$db);
   $result = $connect->query("delete from suppliers where SupplierID ='". $_REQUEST{'SupplierID'} . "'");   
...
if ($act == "edit : confirm") {
  $connect = new mysqli($host,$user,$password,$db);
  $result = $connect->query("update $tb set CompanyName ='". $_REQUEST{'CompanyName'} . "',  ContactName ='". $_REQUEST{'ContactName'} . "' where SupplierID =" . $_REQUEST{'SupplierID'});      
