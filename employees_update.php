...
if (strlen($act) == 0 || $act == "del" || $act == "edit") {
  ...
  while ($o = $result->fetch_object()) {
    ...
    echo "<tr><form action='' method=post>
      ...
      <td><input name=FirstName size=40 value='". $o->FirstName . "' $chg'></td>
      <td><input name=LastName size=20 value='". $o->LastName . "' $chg;text-align:right'></td>
      <td><input name=Title size=20 value='". $o->Title . "' $chg;text-align:right'></td>
      <td>";
  ...
  echo "<tr><form action='' method=post><td><input name=EmployeeID size=5></td><td><input name=FirstName size=40></td><td><input name=LastName size=20></td><td><input name=Title size=20></td><td><input type=submit name=action value='add' style='height:26'></td></tr>
  </form></table>";
...
if ($act == "add") {
  $connect = new mysqli($host,$user,$password,$db);
   $result = $connect->query("insert into $tb (EmployeeID,LastName,FirstName,Title) values('". $_REQUEST{'EmployeeID'} . "','". $_REQUEST{'FirstName'} . "','". $_REQUEST{'LastName'} . "','". $_REQUEST{'Title'} . "')");   
...
if ($act == "del : confirm") {
  $connect = new mysqli($host,$user,$password,$db);
   $result = $connect->query("delete from $tb where EmployeeID ='". $_REQUEST{'EmployeeID'} . "'");   
...
if ($act == "edit : confirm") {
  $connect = new mysqli($host,$user,$password,$db);
  $result = $connect->query("update $tb set FirstName ='". $_REQUEST{'FirstName'} . "', LastName ='". $_REQUEST{'LastName'} . "', Title ='". $_REQUEST{'Title'} . "' where EmployeeID =" . $_REQUEST{'EmployeeID'});      
